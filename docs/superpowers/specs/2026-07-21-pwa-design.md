...
- Geração via um script próprio (`apps/web/scripts/generate-pwa-icons.mjs`, usando `sharp` diretamente) em vez de `@vite-pwa/assets-generator` — mesmo resultado (composição do logo sobre um fundo sólido, escalado por tamanho), mas sem depender da CLI de terceiros, já que o controle exato de escala/respiro por ícone era mais simples de expressar em código direto. Rodado uma vez a partir do logo existente; os PNGs resultantes são committados em `apps/web/public/`.
...
- `registerType: 'autoUpdate'` — ao detectar uma nova build, o service worker assume o controle e recarrega automaticamente, sem perguntar nada ao usuário (decisão explícita: uso pessoal e local, não precisa de prompt de "nova versão disponível").
- **Registro feito à mão em `main.tsx`**, não pelo `injectRegister: 'auto'` do plugin (desvio encontrado durante a verificação manual — ver nota abaixo). `workbox.skipWaiting`/`workbox.clientsClaim` são setados explicitamente em `vite.config.ts`, já que o plugin só os habilita sozinho quando `injectRegister` é `'auto'`.
...
- **Cache de `GET /api/items` e `GET /api/categories`**: `runtimeCaching` com estratégia `NetworkFirst` (tenta a rede primeiro; se falhar — por exemplo, o servidor Fastify não está rodando — cai para a última resposta cacheada). `CatalogPage` depende das duas rotas juntas (`Promise.all`) para renderizar, então as duas precisam estar cacheadas — não só `/api/items` (desvio encontrado durante a verificação manual — ver nota abaixo). Cobre o pedido original: "visualização do catálogo já carregado" quando offline.
- Nenhuma outra rota de API é cacheada: `GET /api/items/:id`, `POST/PATCH/DELETE /api/items`, `POST /api/recommend`, `GET /api/consultas`, uploads — todas continuam exigindo o servidor rodando. Decisão explícita: só a lista do catálogo fica disponível offline, não os detalhes de cada item.
...
> **Nota sobre desvios encontrados na verificação manual**: dois pontos deste design precisaram de ajuste depois de testados de verdade (navegador real, servidor real, `context.setOffline`/rebuild reais via Playwright), não só lidos no código: (1) cachear só `/api/items` deixava a tela de catálogo inteira quebrada offline, porque ela também depende de `/api/categories`; (2) `injectRegister: 'auto'` sozinho não verifica atualizações proativamente nem garante que um worker novo tome o controle de uma aba já aberta — o registro precisou ser feito à mão, com `skipWaiting`/`clientsClaim` explícitos no `workbox` config. Ambos corrigidos e reverificados ao vivo antes de fechar a tarefa.
