...
Quatro arquivos na raiz do repositório, fora de `apps/server` e `apps/web` porque orquestram os dois workspaces:
...
- **`launch.bat`**: se `apps/web/dist` não existir, roda `npm run build -w apps/web`. Em seguida, faz um `GET http://localhost:3001/api/health`:
  ...
  - Se não responder → sobe o servidor (`tsx src/server.ts` dentro de `apps/server`) e então abre o navegador na mesma URL.
